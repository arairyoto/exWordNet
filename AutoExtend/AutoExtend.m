function [] = AutoExtend(varargin)
    ...
    trainModel(folder, dim, num_iters, countSynsets, countWords, F, W, Theta, Iota, R, weights, settings, experiment);
end

function [] = trainModel(folder, dim, num_iters, countSynsets, countWords, F, W, Theta, Iota, R, weights, settings, experiment)
    ...
    for d=1:dim %parfor
        ...
        if exist(dimFilename, 'file')
            ...
            [Theta_dim, Iota_dim, J_history_dim, lastNormIter_dim] = trainDimension(num_iters, countSynsets, countWords, F, w, Theta, Iota, R, weights, settings, debugFile);
            saveVariables(dimFilename, Theta_dim, Iota_dim, J_history_dim, lastNormIter_dim);
...
function [EValues, DValues, J_history, lastNormIter] = trainDimension(num_iters, ~, countWords, F, w, E, D, R, weights, settings, debugFile)
    ...
    while iter <= num_iters
        ...
        if (weights(1) > 0)
            [J1, grads_E, grads_D] = gradient(F, w, E, D, w, 'both');
        ...
        if (weights(3) > 0)
            [J3, gradr_E, ~] = gradient(F, w, E, R, zeros(size(R,1),1) , 'onlyE');
